...
- **Session keep-alive:** server-rendered pages don't instantiate a browser Supabase client, so nothing ran token auto-refresh — the 1h access token aged out during idle and soft navigations silently failed (a token rotated during an RSC render can't write cookies). `components/layout/SessionWatcher.tsx` (mounted in the authed `(app)/layout`) keeps one long-lived browser client alive (auto-refresh), `router.refresh()`es on `TOKEN_REFRESHED`, redirects to `/login` on `SIGNED_OUT`, and re-validates the session on tab focus.
...
- **Peer count is public:** `match_count(p_user)` SECURITY DEFINER fn (count only, never who) — shown as "👥 Peers" on `/profile` and `/users/[id]`. The **Peers** list (`/matches`, client component `components/dating/PeersList.tsx`) links each row to the person's profile + a message button (`/messages/[matchId]`), with a client-side search box that filters your peers by name/@username.
...
Header bell (`NotifBell`) → `/api/notifications` (unread count). `/notifications` page lists posts by users you're connected with (`lib/notifications.ts::getNotifications`, derived on read from `matches`, excludes blocked). Opening it POSTs `/api/notifications/seen` (sets `last_notifications_seen`). **Deletable:** there's no notification table, so dismissing one records the post in `dismissed_notifications` (migration 022, de-junctioned in 023) via `POST /api/notifications/dismiss`; `getNotifications` filters those out. UI is `components/notifications/NotificationsList.tsx` — swipe-left on mobile, tap-to-reveal View/Delete on desktop, optimistic removal.
