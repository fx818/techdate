...
- **`proxy.ts` (NOT `middleware.ts`):** Next.js 16 renamed middleware → `proxy.ts` (root). It redirects unauthenticated → `/login` (except public routes `/login`, `/onboarding`, `/posts`), authenticated at `/` → `/feed`, and enforces the company-email gate for **personal-email** users on soft navigations. Matcher **excludes `_next/static`, `_next/image`, `favicon.ico`, `api`, `auth`, AND the PWA static files `sw.js|swe-worker|manifest.webmanifest|icon-192.png|icon-512.png`** (added 2026-06-26 — without these the guard 307-redirects `/sw.js`→`/login` and the service worker never registers; see [push](arch-push.md)); API routes do their own `getUser()` checks. (CLAUDE.md still says "middleware.ts" — stale.)
