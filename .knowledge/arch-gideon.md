...
type: architecture
title: Gideon Agent
description: Python cron seeding posts from 6 sources with normalized ranking, an optional LLM quality gate, and an admin reject-review queue
...
- **Pipeline (fetch.py `run()`):** `merge_normalized` → `dedup_candidates` (URL + slugified-title dedup, **also excludes tombstoned URLs**) → judge (if active) else `unique[:MAX]` → `insert_records`; when the judge is active its DROPs are recorded via `record_rejections`.
...
- **Reject queue — `gideon_rejections` (migration 032):** `select_with_judge` returns `(kept, dropped)`; the drops it made during the backfill walk are written to `gideon_rejections` (score + reason) by `record_rejections`, skipping URLs already live or already queued (`load_live_gideon_urls` ∪ `load_queued_reject_urls`). The admin reviews them at [/admin/gideon/rejections](arch-moderation.md): **Approve** (`gideon_approve_rejection` RPC → inserts into `posts`) or **Delete** (`gideon_dismiss_rejection` RPC → tombstones the URL in `gideon_dismissed_urls`). Tombstoned URLs are excluded from `dedup_candidates` so they never re-post or re-queue. Un-actioned rejects auto-expire after 14 days (`purge_expired_rejections`, once per run). Rejects are captured **only when the judge is active**.
