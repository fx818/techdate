...
- **CRITICAL `proxy.ts` matcher gotcha:** the auth guard's matcher MUST exclude `sw.js|swe-worker|manifest.webmanifest|icon-192.png|icon-512.png` (alongside `_next/static|_next/image|favicon.ico|api|auth`). Otherwise the guard 307-redirects `/sw.js`→`/login` for logged-out visits and the service worker never registers. Verified live post-deploy: `/sw.js`, `/manifest.webmanifest`, `/icon-{192,512}.png` all 200; `/feed` still 307→/login. This bit us at deploy time — static review can't catch it (runtime/routing behavior).
